...
function render(L){document.documentElement.lang=L;document.querySelector('#headerTitle').textContent=names[L];document.querySelector('#welcomeTitle').textContent=intro[L][0];document.querySelector('#welcomeText').textContent=intro[L][1];lang.value=L;nav.innerHTML='';list.innerHTML='';A.forEach(x=>{const [n,title,en,hi,mr]=x;const b=document.createElement('button');b.textContent=`${n}. ${title}`;b.onclick=()=>document.getElementById(`a${n}`).scrollIntoView({behavior:'smooth'});nav.appendChild(b);const card=document.createElement('article');card.className='aarti';card.id=`a${n}`;card.innerHTML=`<div class="number">AARTI ${n}</div><h3>${[en,hi,mr][0+({en:0,hi:1,mr:2}[L])]}</h3><div class="divider"></div><div class="lyrics"></div><div class="note">॥ श्री गणेशाय नमः ॥</div>`;card.querySelector('.lyrics').textContent=[en,hi,mr][{en:0,hi:1,mr:2}[L]];list.appendChild(card);});}
function openBook(L){localStorage.setItem('aartiLang',L);document.querySelector('#languageScreen').classList.add('hidden');document.querySelector('#bookScreen').classList.remove('hidden');render(L);window.scrollTo(0,0)}
document.querySelectorAll('[data-lang]').forEach(b=>b.onclick=()=>openBook(b.dataset.lang));lang.onchange=e=>render(e.target.value);document.querySelector('#home').onclick=()=>{document.querySelector('#bookScreen').classList.add('hidden');document.querySelector('#languageScreen').classList.remove('hidden');window.scrollTo(0,0)};
const saved=localStorage.getItem('aartiLang'); if(saved) openBook(saved);
